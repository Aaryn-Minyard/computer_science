...
def get_map_representation(current_room: Room) -> str:
    """Generate a simple map representation of the dungeon."""
    map_layout = """
    [E]---[H]---[T]
              |
              [A]---[N1]---[N2]---[N3]
    ...
    # Labels to represent rooms based on their names
    map_legend = {
        "Entrance": "E",
        "Hallway": "H",
        "Treasure Room": "T",
    ...
    # Highlight the current room with an asterisk (*)
    current_room_label = map_legend.get(current_room.name, "?")
    map_with_highlight = map_layout.replace(f"[{current_room_label}]", f"*{current_room_label}*")
...
def model(user_inputs: Iterable[str], current_room: Room) -> Generator[str, None, None]:
    for user_input in user_inputs:
        ...
        if command in [Command.NORTH, Command.SOUTH, Command.EAST, Command.WEST]:
            ...
            next_room = move(current_room, direction)  # Move to the next room
            ...
            yield party.view_party()
            yield get_map_representation(current_room)
